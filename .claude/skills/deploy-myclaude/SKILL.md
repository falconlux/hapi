...
PYEOF
cd assets
OLD_JS=$(ls index-*.js 2>/dev/null | grep -v "\-my\." | head -1 || echo "")
OLD_CSS=$(ls index-*.css 2>/dev/null | grep -v "\-my\." | head -1 || echo "")
if [ -n "$OLD_JS" ]; then NEW_JS="${OLD_JS%.js}-my.js"; mv "$OLD_JS" "$NEW_JS"; cd ..; sed -i "s|$OLD_JS|$NEW_JS|g" index.html sw.js 2>/dev/null; cd assets; fi
if [ -n "$OLD_CSS" ]; then NEW_CSS="${OLD_CSS%.css}-my.css"; mv "$OLD_CSS" "$NEW_CSS"; cd ..; sed -i "s|$OLD_CSS|$NEW_CSS|g" index.html sw.js 2>/dev/null; fi
cd ~/hapi-myclaude/web/dist
