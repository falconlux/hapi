...
export function registerSessionHandlers(socket: CliSocketWithData, deps: SessionHandlersDeps): void {
    const { store, resolveSessionAccess, emitAccessError, onSessionAlive, onSessionEnd, onSessionUsage, onWebappEvent, onBackgroundTaskDelta, onSessionActivity } = deps
    ...
    socket.on('message', async (data: unknown, ack?: () => void) => {
        ...
        if (shouldRecordSessionActivity(rehosted)) {
        ...
        const bgDelta = extractBackgroundTaskDelta(rehosted)
        if (bgDelta) {
