...
- MCP `create_peer` creates a separate visible Codex session on the current runner and namespace. Required arguments: `title`, `cwd`, and either `initialMessage` or `objective`; optional: `model`, `reasoningEffort`. It returns the stable HAPI `sessionId`, rejects empty/recent duplicate creations, and links the child to the creating session. The child is instructed to `ping_peer` checkpoints/final status to its manager, while the Hub sends an idempotent fallback notification when the child ends.
