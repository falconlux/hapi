    ...
    startHappyServer: vi.fn(async (_client: unknown, options: { skillLookup?: unknown }) => {
        ...
        return {
            ...
            toolNames: options.skillLookup
                ? ['change_title', 'display_image', 'display_video', 'display_media', 'list_peers', 'create_peer', 'ping_peer', 'inspect_peer', 'list_project_groups', 'create_project_group', 'rename_project_group', 'delete_project_group', 'move_sessions_to_group', 'rename_peer', 'archive_peer', 'unarchive_peer', 'skill_lookup']
                : ['change_title', 'display_image', 'display_video', 'display_media', 'list_peers', 'create_peer', 'ping_peer', 'inspect_peer', 'list_project_groups', 'create_project_group', 'rename_project_group', 'delete_project_group', 'move_sessions_to_group', 'rename_peer', 'archive_peer', 'unarchive_peer'],
            stop: vi.fn()
        }
    })
}))
...
describe('buildHapiMcpBridge skill lookup config', () => {
    ...
    it('forwards the enabled HTTP tool through STDIO and auto-approves it', async () => {
        ...
        expect(bridge.mcpServers.hapi.tools).toEqual({
            ...
            list_project_groups: { approval_mode: 'approve' },
            create_peer: { approval_mode: 'prompt' },
            create_project_group: { approval_mode: 'prompt' },
            rename_project_group: { approval_mode: 'prompt' },
            delete_project_group: { approval_mode: 'prompt' },
            move_sessions_to_group: { approval_mode: 'prompt' },
            rename_peer: { approval_mode: 'prompt' },
            archive_peer: { approval_mode: 'prompt' },
            unarchive_peer: { approval_mode: 'prompt' },
    ...
    it('does not expose skill_lookup for native-skill bridge callers', async () => {
        ...
        expect(harness.cliArgs.at(-1)).toBe('change_title,display_image,display_video,display_media,list_peers,create_peer,ping_peer,inspect_peer,list_project_groups,create_project_group,rename_project_group,delete_project_group,move_sessions_to_group,rename_peer,archive_peer,unarchive_peer')
        expect(bridge.mcpServers.hapi.tools).toEqual({
            change_title: { approval_mode: 'approve' },
            display_image: { approval_mode: 'prompt' },
            display_video: { approval_mode: 'prompt' },
