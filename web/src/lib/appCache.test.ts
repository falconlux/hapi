...
describe('clearAppCacheAndReload', () => {
    ...
    it('clears Cache Storage and message windows while preserving sign-in and preferences', async () => {
        ...
        sessionStorage.setItem('hapi:message-window:v2:session-b', 'cached-b')
        sessionStorage.setItem('tsr-scroll-restoration-v1_3', 'scroll')
        sessionStorage.setItem('hapi:composer-drafts', 'draft')
        localStorage.setItem('tsr-scroll-restoration-v1_2', 'scroll')
        localStorage.setItem('hapi_access_token::https://example.test', 'token')
        localStorage.setItem('hapi-lang', 'zh-CN')

        await clearAppCacheAndReload({ cacheStorage, localStorage, sessionStorage, reload })
        ...
        expect(sessionStorage.getItem('hapi:message-window:v2:session-b')).toBeNull()
        expect(sessionStorage.getItem('tsr-scroll-restoration-v1_3')).toBeNull()
        expect(localStorage.getItem('tsr-scroll-restoration-v1_2')).toBeNull()
