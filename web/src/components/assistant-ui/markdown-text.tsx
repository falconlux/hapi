...
function Image(props: ComponentPropsWithoutRef<'img'>) {
    ...
    if (!url) {
        return <img {...props} className={cn('aui-md-img my-3 max-w-full rounded-xl', className)} />
    }
    return (
        <ImageLightbox src={url} alt={altText} downloadUrl={url} filename={altText || 'image'}>
            <img
                ...
                src={url}
                alt={altText}
                className={cn('aui-md-img my-3 max-w-full rounded-xl cursor-zoom-in', className)}
            />
        </ImageLightbox>
    )
}
